...
class DemoActivity : AppCompatActivity() {
    ...
    // Selected Image array list
    val arrImageList: ArrayList<Drawable?> = ArrayList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = RecyclerViewAdapter(arrImageList)
        ...
        picker.listener = object : BubblePickerListener {
            override fun onBubbleSelected(item: PickerItem) {
                arrImageList.add(item.icon)
                adapter.notifyDataSetChanged()
            }

            override fun onBubbleDeselected(item: PickerItem) {
                if (arrImageList.contains(item.icon)) {
                    arrImageList.remove(item.icon)
                }
                adapter.notifyDataSetChanged()
            }
        }
